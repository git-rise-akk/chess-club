export default class Carousel {
  constructor({
    ...
    this.dotsContainerSelector = dotsContainer;
  ...
  initDots() {
    this.dotsContainer = this.container.querySelector(this.dotsContainerSelector);
    ...
    this.dots = this.dotsContainer.querySelectorAll(`.${this.dotClass}`);
  }
  ...
  updateDots() {
    ...
    this.dots.forEach((dot, idx) => {
      dot.classList.toggle(this.dotActiveClass, idx === activeIndex);
    });
  }
  ...
  updateUI() {
    if (this.counterCurrent) {
      ...
      if (this.infinite) displayIndex -= this.visibleCount;
  ...
  addEvents() {
    ...
    window.addEventListener('resize', () => {
      ...
      this.resizeTimer = setTimeout(() => this.recalc(), 100);
    });
  }
}